...
public class DataMD {
    ...
    public DataMD(String file, int n, int nDepot, int l){
        try {
            ...
            this.coor_x = new double[nNode+nDepot];
            this.coor_y = new double[nNode+nDepot];
            this.demand = new int[nNode+nDepot+1];
            this.tw_a = new int[nNode+nDepot+1];
            this.tw_b = new int[nNode+nDepot+1];
            this.service_time = new int[nNode+nDepot+1];
...
//                this.service_time[i] = 0;//Integer.parseInt(entry[6]);
//            }
            this.tw_a[nNode+nDepot] = this.tw_a[0];
            this.tw_b[nNode+nDepot] = this.tw_b[0];
            this.demand[nNode+nDepot] = this.demand[0];
            this.service_time[nNode+nDepot] = 0;//= this.demand[0];

            this.distance = new double[nDepot][nNode+1][nNode+1];
            this.cost = new double[nDepot][nNode+1][nNode+1];
            for(int k = 0; k < nDepot; k++) {
                for(j = 1; j < nNode; j++){
                    this.distance[k][0][j] = haversine(k, j+nDepot);
                    this.distance[k][j][0] = this.distance[k][0][j];
                }
                for (i = 1; i < nNode; i++) {
                    for (j = i; j < nNode; j++) {
                        this.distance[k][i][j] = haversine(i+nDepot, j+nDepot);
                        this.distance[k][j][i] = this.distance[k][i][j];
                    }
                    this.distance[k][i][nNode] = haversine(i+nDepot, k);
